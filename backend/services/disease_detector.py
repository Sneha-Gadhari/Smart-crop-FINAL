...
class DiseaseDetector:
    ...
    def _load(self):
        try:
            ...
            print(f"  Downloading disease model from {self.REPO} ...")
            print(f"  HF token: {'set' if self.token else 'not set (public repo — OK)'}")

            # token=None works fine for public repos
            ...
            print(f"  Model files downloaded/cached: {weights_path}")
            ...
            print(f"  Config: {len(self.class_names)} classes, {img_size}px, arch={arch}")
            ...
            print(f"✅ Disease model loaded: {self.REPO} ({len(self.class_names)} classes)")

        except Exception as e:
            print(f"⚠️  Disease model load failed: {e}")
            print(f"    → Running in mock mode. Scan will return demo data.")
            print(f"    → Fix: ensure internet connection on first startup for HF download.")
